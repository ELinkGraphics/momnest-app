      ...
      {post.post_type === 'pdf' && post.media?.urls && post.media.urls.length > 0 ? (
        <div className="px-4 pb-2">
          ...
          >
            ...
            <div className="relative aspect-[4/5] rounded-xl overflow-hidden shadow-lg border border-primary/5 bg-black/5">
              <img
                src={post.media.urls[0]}
                alt={post.content || "PDF document preview"}
                className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
              />
              
              {/* PDF Badge Overlay */}
              <div className="absolute top-3 left-3 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-black/60 backdrop-blur-md border border-white/10 text-white">
                <FileText className="w-4 h-4 text-primary" />
                <span className="text-[10px] font-bold uppercase tracking-wider">PDF</span>
              </div>

              {/* Page Count Badge */}
              {post.media.urls.length > 0 && (
                <div className="absolute bottom-3 right-3 px-2 py-1 rounded-md bg-black/40 backdrop-blur-sm text-white/90 text-[10px] font-medium">
                  {post.media.urls.length} Pages
                </div>
              )}

              {/* Hover Effect Overlay */}
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-colors duration-300 flex items-center justify-center">
                <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300 px-4 py-2 rounded-full bg-primary/90 text-white text-xs font-semibold shadow-xl">
                  View Document
                </div>
              </div>
            </div>
